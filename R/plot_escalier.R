...
#' \code{\link{build_geometry}} (ou d'une ligne de
...
#'
#' Les géométries produites par \code{\link{build_geometry}} portent la
...
#'
#' @param geometrie Un data.frame de géométrie (voir \code{\link{build_geometry}}).
...
plot_escalier <- function(geometrie, col_standard = "black", col_paliere = "red",
  ...
  x_max <- max(geometrie$x_going_end, geometrie$x_riser, na.rm = TRUE)
  y_max <- max(geometrie$y_top , na.rm = TRUE)
  y_min <- min(geometrie$y_bottom, na.rm = TRUE)
  ...
  for (i in seq_len(nrow(geometrie))) {
    ...
    graphics::segments(
      geometrie$x_riser[i], geometrie$y_bottom[i],
      geometrie$x_riser[i], geometrie$y_top [i],
      lwd = 2
    )

    # Giron (segment horizontal), absent pour la dernière ligne (arrivée palier)
    if (!is.na(geometrie$going[i])) {
      couleur <- if (identical(geometrie$going_type[i], "paliere")) col_paliere else col_standard
      graphics::segments(
        geometrie$x_riser[i], geometrie$y_top [i],
        geometrie$x_going_end[i], geometrie$y_top [i],
        lwd = 2, col = couleur
      )
    }
  }
  ...
  if (cotes) {
    ...
    # Côte horizontale cumulée : x_going_end de chaque marche, sous l'escalier
    y_ligne_cote_x <- y_min - marge_y * 0.4
    y_texte_cote_x <- y_min - marge_y * 0.9
    for (i in seq_len(nrow(geometrie))) {
      if (!is.na(geometrie$x_going_end[i])) {
        graphics::segments(geometrie$x_going_end[i], y_min, geometrie$x_going_end[i], y_ligne_cote_x,
                  lty = 3, col = "gray60")
        graphics::text(geometrie$x_going_end[i], y_texte_cote_x, round(geometrie$x_going_end[i], 1),
             srt = 90, cex = cex_cotes, adj = 1, col = "gray30", xpd = TRUE)
      }
    }

    # Côte verticale cumulée : y_top  de chaque contremarche, à gauche de l'escalier
    x_ligne_cote_y <- 0 - marge_x * 0.4
    x_texte_cote_y <- 0 - marge_x * 0.9
    for (i in seq_len(nrow(geometrie))) {
      graphics::segments(0, geometrie$y_top [i], x_ligne_cote_y, geometrie$y_top [i],
                lty = 3, col = "gray60")
      graphics::text(x_texte_cote_y, geometrie$y_top [i], round(geometrie$y_top [i], 1),
...
#' data.frame.
#'
#' @param geometry Un data.frame de géométrie (voir \code{\link{build_geometry}}).
...
#' produite par \code{\link{build_geometry}} (classe
