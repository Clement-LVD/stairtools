...
followed by the graphical parameters provided by
...
[`solve_stairs()`](https://clement-lvd.github.io/stairtools/reference/solve_stairs.md)
compute various basic stair solutions, given a total height and a
maximum horizontal run available. The function returns a `data.frame`
listing several solutions, the most relevant of which is the first
entry. A `list` variable called `geometry` provides the coordinates for
each solution, with one entry for each solution in the main data.frame,
in the same order.
...
By default,
[`solve_stairs()`](https://clement-lvd.github.io/stairtools/reference/solve_stairs.md)
return basic concrete stairs, computed with
...
[`plot_stair()`](https://clement-lvd.github.io/stairtools/reference/plot_stair.md)
uses base R’s
[`graphics::polygon()`](https://rdrr.io/r/graphics/polygon.html) to draw
each stair surface. Graphical parameters can therefore be passed through
`polygon_params`.
...
also be used, including line types and hatching.
