# Package index

## All functions

- [`add_stair_comfort_values()`](https://clement-lvd.github.io/stairtools/reference/add_stair_comfort_values.md)
  : Add stair comfort indicators
- [`best_solution()`](https://clement-lvd.github.io/stairtools/reference/best_solution.md)
  : Select the best stair solution
- [`blondel_going()`](https://clement-lvd.github.io/stairtools/reference/blondel_going.md)
  : Compute the going from Blondel's formula
- [`build_geometry()`](https://clement-lvd.github.io/stairtools/reference/build_geometry.md)
  : Build the complete geometry of a stair
- [`build_solutions_table()`](https://clement-lvd.github.io/stairtools/reference/build_solutions_table.md)
  : Build the complete table of stair solutions (embedded geometry)
- [`check_stair_rules()`](https://clement-lvd.github.io/stairtools/reference/check_stair_rules.md)
  : Check stair solutions against dimensional rules
- [`compute_geom_layout()`](https://clement-lvd.github.io/stairtools/reference/compute_geom_layout.md)
  : Compute the rendered stair profile from a stair geometry
- [`generate_going_scenarios()`](https://clement-lvd.github.io/stairtools/reference/generate_going_scenarios.md)
  : Generate possible going scenarios for a given number of steps
- [`geometry_to_scene()`](https://clement-lvd.github.io/stairtools/reference/geometry_to_scene.md)
  : Convert stair geometry into a plotting scene
- [`optimal_nrisers()`](https://clement-lvd.github.io/stairtools/reference/optimal_nrisers.md)
  : Determine feasible numbers of risers (vertical element between
  steps) for a given total height
- [`plot(`*`<stair_scene>`*`)`](https://clement-lvd.github.io/stairtools/reference/plot.stair_scene.md)
  : Plot a stair scene
- [`plot_stair()`](https://clement-lvd.github.io/stairtools/reference/plot_stair.md)
  [`plot(`*`<stair_geometry>`*`)`](https://clement-lvd.github.io/stairtools/reference/plot_stair.md)
  : Plot a stair profile
- [`plot_stair_dimensions()`](https://clement-lvd.github.io/stairtools/reference/plot_stair_dimensions.md)
  : Plot a stair profile with dimensions
- [`print(`*`<stair_solutions>`*`)`](https://clement-lvd.github.io/stairtools/reference/print.stair_solutions.md)
  : Print stair solutions without the geometry list-column
- [`scene_add_dimensions()`](https://clement-lvd.github.io/stairtools/reference/scene_add_dimensions.md)
  : Add overall going and rise dimension lines to a stair scene
- [`scene_add_risers()`](https://clement-lvd.github.io/stairtools/reference/scene_add_risers.md)
  : Add riser segments to a stair scene
- [`scene_add_treads()`](https://clement-lvd.github.io/stairtools/reference/scene_add_treads.md)
  : Add tread segments to a stair scene
- [`scene_build()`](https://clement-lvd.github.io/stairtools/reference/scene_build.md)
  : Initialize an empty stair plotting scene
- [`solve_stairs()`](https://clement-lvd.github.io/stairtools/reference/solve_stairs.md)
  : Compute all possible stair solutions
- [`stair_terminology`](https://clement-lvd.github.io/stairtools/reference/stair_terminology.md)
  : Stair terminology
- [`theme_default()`](https://clement-lvd.github.io/stairtools/reference/theme_default.md)
  : Default per-role styling for a stair scene
