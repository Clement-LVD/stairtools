...
`step`, `x_step_start`, `y_bottom`, `y_top`, `going`, `x_going_end`, and
`going_type`.
...
- Riser *i* (vertical): from `(x_step_start, y_bottom)` to
  `(x_step_start, y_top)`.

- Going *i* (horizontal), when present: from `(x_step_start, y_top)` to
  `(x_going_end, y_top)`.
